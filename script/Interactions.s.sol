...
contract CreateSubscription is Script {
    ...
    function createSubscription(address vrfCoordinator, uint256 deployerKey) public returns (uint64) {
        ...
        uint64 subId = VRFCoordinatorV2Interface(vrfCoordinator).createSubscription();
        vm.stopBroadcast();
...
contract FundSubscription is Script {
    ...
    function fundSubscription(address vrfCoordinator, uint64 subId, address link, uint256 deployerKey) public {
        ...
        if (block.chainid == 31337) {
            //本地开发网
            vm.startBroadcast();
            VRFCoordinatorV2Mock(vrfCoordinator).fundSubscription(subId, FUND_ACOUNT);
            vm.stopBroadcast();
        } else {
            vm.startBroadcast(deployerKey);
            LinkToken(link).transferAndCall(vrfCoordinator, FUND_ACOUNT, abi.encode(subId));
            vm.stopBroadcast();
        }
    }
...
contract AddConsumer is Script {
    function addConsume(address raffle, address vrfCoordinator, uint64 subId, uint256 deployerKey) public {
    ...
    function addConsumerUsingConfig(address raffle) public {
        HelpConfig helpConfig = new HelpConfig();
        (,, address vrfCoordinator,,, uint64 subId,, uint256 deployerKey) = helpConfig.activeNetworkConfig();
        addConsume(raffle, vrfCoordinator, subId, deployerKey);
    }

    function run() external {
        address raffle = DevOpsTools.get_most_recent_deployment("Raffle", block.chainid); //动态查找最近部署的合约地址
